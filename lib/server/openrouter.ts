...
import { checkCircuit, recordCircuitFailure, recordCircuitSuccess } from "@/lib/server/circuit-breaker";
...
const isCircuitStatus = (status: number) => status === 429 || status >= 500;
const circuitKey = "openrouter";
...
const createCircuitOpenError = (retryAfterMs: number) => {
  const retrySeconds = Math.max(1, Math.ceil(retryAfterMs / 1000));
  return new OpenRouterError({
    message: `AI provider temporarily unavailable. Retry in ${retrySeconds}s.`,
    code: "circuit_open",
    upstreamStatus: 503,
  });
};
...
export const callOpenRouterChat = async (params: {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const circuit = checkCircuit(circuitKey);
    if (circuit.open) {
      throw createCircuitOpenError(circuit.retryAfterMs);
    }
    ...
    if (!response.ok) {
      ...
      if (isCircuitStatus(response.status)) {
        recordCircuitFailure(circuitKey);
      }
